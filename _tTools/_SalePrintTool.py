        ...
        if 'payment_error' not in p.keys() and 'process_error' not in p.keys():
            if 'topup_details' in p.keys():
                ...
                if 'refund_status' in p.keys() and p['refund_status'] == 1:
                    pdf.ln(small_space)
                    pdf.set_font(USED_FONT, '', regular_space)
                    pdf.cell(padding_left, 0, 'AKUN WHATSAPP   : ' + p['refund_number'], 0, 0, 'L')
                    pdf.ln(small_space)
                    pdf.set_font(USED_FONT, '', regular_space)
                    pdf.cell(padding_left, 0, 'SALDO DIALIHKAN : Rp. ' + clean_number(str(p['refund_amount'])), 0, 0, 'L')
                pdf.ln(small_space*2)
    ...
    try:
        ...
        if 'payment_error' not in p.keys() and 'process_error' not in p.keys():
            if p['ppob_mode'] == 'tagihan':
                ...
                pdf.ln(small_space)
                pdf.set_font(USED_FONT, '', regular_space)
                pdf.cell(padding_left, 0, 'BESAR TAGIHAN : Rp. ' + clean_number(str(p['value'])), 0, 0, 'L')
                pdf.ln(small_space)
                pdf.set_font(USED_FONT, '', regular_space)
                ...
                pdf.ln(small_space)
                pdf.set_font(USED_FONT, '', regular_space)
                pdf.cell(padding_left, 0, 'HARGA SATUAN  : Rp. ' + clean_number(str(p['value'])), 0, 0, 'L')
                if 'sn' in p['ppob_details'].keys():
                    pdf.ln(small_space)
                    pdf.set_font(USED_FONT, '', regular_space)
                    pdf.cell(padding_left, 0, 'TOKEN/SN  : ' + str(p['ppob_details']['sn'][:24]), 0, 0, 'L')
            if 'refund_status' in p.keys() and p['refund_status'] == 1:
                pdf.ln(small_space)
                pdf.set_font(USED_FONT, '', regular_space)
                pdf.cell(padding_left, 0, 'AKUN WHATSAPP   : ' + p['refund_number'], 0, 0, 'L')
